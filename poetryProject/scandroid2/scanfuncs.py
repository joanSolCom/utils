...
class ScansionMachine:
    ...
    def execute(self, text):
		...
		Steps = [('SYLLABLES', self.ShowSyllables), \
		              ('PRELIMINARY MARKS', self.ShowLexStresses)]
		if iambic:
		    Steps.append(('CHOOSE ALGORITHM', self.ChooseAlgorithm))
		    if algorithm1:
		        Steps.append(('FIRST TESTS', self.WeirdEnds))
		        Steps.append(('FOOT DIVISION', self.TestLengthAndDice))
		    else:
		        Steps.append(('LONGEST NORMAL', self.TryREs))
		        Steps.append(('CLEAN UP ENDS', self.CleanUpRE))
		    Steps.append(('PROMOTIONS', self.PromotePyrrhics))
		    Steps.append(('ANALYSIS', self.HowWeDoing))
		else:		# anapestic steps
		    Steps.append(('ADJUST STRESSES', self.GetBestAnapLexes))
		    Steps.append(('ANAPESTICS: LINE END', self.AnapEndFoot))
		    Steps.append(('ANAPESTICS: FOOT DIVISION', self.AnapDivideHead))
		    Steps.append(('ANAPESTICS: ANALYSIS', self.AnapCleanUpAndReport))

		for step in Steps:
			(scanline, result) = step[1]()

		return self.allSyls, self.P.GetMarks()
...
## - - - - - first visible steps of ANY scansion
    def ShowSyllables(self, logger=None):
    ...
    def ShowLexStresses(self, logger=None):
...
## - - - - - where iambic and anapestic steps diverge
    def ChooseAlgorithm(self, logger=None, deducingParams=False):
...
## - - if one iambic algorithm fails, we try the other, starting here with a restart
    def RestartNewIambicAlg(self, logger=None, failedAlgorithm=None, scanline=None):
...
## - - - - - - - - iambic Algorithm 1: Corral the Exceptional - - - - - - - - - - - 
    def WeirdEnds(self, logger=None):
    ...
    def TestLengthAndDice(self, logger=None):
...
## - - - - - - - - iambic Algorithm 2: Maximize the Normal - - - - - - - - - - - - 
    def TryREs(self, logger=None):
        ...
        for (footname, sylinx) in footfinder(footDict, marks, 2, startlongest, runend):
            ...
            if sylinx < len(marks): 
                self.P.AddFootDivMark(sylinx)
        #logger.ExpREMain(startlongest, longest, tail, len(lineDat['footlist']), lineDat['lfeet'])
        return retstr, True

    def CleanUpRE(self, logger=None):
...
## - - - - - - - - end of fork between iambic Algorithms 1 & 2 - - - - - - - - - 
    def PromotePyrrhics(self, logger=None):
    ...
    def HowWeDoing(self, logger=None):
...
## - - - - - below, all code for scanning anapestics quickly and step-by-step
    def GetBestAnapLexes(self, logger=None, deducingParams=False):
        ...
        from the Positioner a list of altenative lexical-stress lines, each with
    ...
    def AnapEndFoot(self, logger=None):
    ...
    def AnapDivideHead(self, logger=None):
    ...
    def AnapCleanUpAndReport(self, logger=None):
