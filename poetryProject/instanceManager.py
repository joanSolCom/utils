...
import codecs
import nltk
from scandroid2.scanfuncs import ScansionMachine
...
class Instance:
	...
	def __init__(self, name, label, paths):
		...
		stanzaList = self.text.split("\n\n")

		for stanza in stanzaList:
			stanzaStruct = {}
			stanzaStruct["stanza"] = stanza
			stanzaStruct["verses"] = []
			verses = stanza.split("\n")

			for verse in verses:
				verseStruct = {}
				verseStruct["verse"] = verse
				syllablesPerWordStressed, stress = SM.execute(verse)
				verseStruct["stress"] = stress
				verseStruct["syllablesPerWord"] = []
				verseStruct["syllableList"] = []
				tokens = verse.split()
				verseStruct["tokens"] = []
				...
				for token in tokens:
					...
					if token:
						verseStruct["tokens"].append(token)
						tokenSyllables = SM.S.Syllabize(token)
						verseStruct["syllablesPerWord"].append(tokenSyllables)
						verseStruct["syllableList"].extend(tokenSyllables)

				stanzaStruct["verses"].append(verseStruct)
			
			self.stanzas.append(stanzaStruct)
...
class InstanceCollection:
	...
	def __init__(self):
		...
		self.labels = set()
		self.instanceDict = {}
		self.featurePath = "./precalculated/"
